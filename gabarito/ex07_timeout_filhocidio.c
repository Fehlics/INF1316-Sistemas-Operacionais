...
{
    (void)sinal;
    /* [resolvido] marque estourou = 1 e mate o filho: kill(filho, SIGKILL) */
    estourou = 1;
    kill(filho, SIGKILL);
}
...
int main(int argc, char *argv[])
{
    ...
    if (filho == 0) {
        /* [resolvido] FILHO: execvp(argv[2], &argv[2]); se retornar, perror + _exit(127). */
        execvp(argv[2], &argv[2]);
        perror("execvp");
        _exit(127);
    }

    /* [resolvido] PAI: signal(SIGALRM, trata_alarme); alarm(limite); */
    signal(SIGALRM, trata_alarme);
    alarm(limite);
    while (waitpid(filho, &status, 0) < 0 && errno == EINTR)
        ;
    alarm(0);
