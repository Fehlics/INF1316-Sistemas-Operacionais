 ...
 */
static int solicitar_syscall(int fd, int id, Operacao operacao, int pc, int n) {
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        fprintf(stderr, "Uso: Application ID FD_SYSCALL\n");
        return 1;
    }

    int id = atoi(argv[1]);
    int fd_syscall = atoi(argv[2]);
    if (id < 1 || id > QUANTIDADE_APLICACOES || fd_syscall < 0) {
    ...
    while (pc < MAX_ITERACOES) {
        ...
        if (operacao != NENHUMA_OPERACAO &&
            solicitar_syscall(fd_syscall, id, operacao, pc, n) == -1) {
            close(fd_syscall);
            return 1;
        }
