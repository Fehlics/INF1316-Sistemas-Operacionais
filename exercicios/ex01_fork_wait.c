...
int main(void)
{
    ...
    // TODO: 1) chame fork() e guarde o retorno em 'pid'.
    //       Se falhar (pid < 0): perror("fork") e exit(1).
    // >>> escreva seu codigo aqui <<<

    if (pid == 0) {
        // TODO: 2) FILHO: imprima getpid() e getppid(); mude x para 5; imprima x;
        //       encerre com exit(3).
        // >>> escreva seu codigo aqui <<<
    }

    // TODO: 3) PAI: espere o filho com waitpid(pid, &status, 0).
    //       Se WIFEXITED(status), imprima WEXITSTATUS(status) (deve ser 3).
    // >>> escreva seu codigo aqui <<<
